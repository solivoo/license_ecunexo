...
public sealed class ClearGrantTenantOverrideHandler
    : ICommandHandler<ClearGrantTenantOverrideCommand, GrantTenantEntitlementsResponse>
{
    private readonly IValidator<ClearGrantTenantOverrideCommand> _validator;
    private readonly ILicenseGrantRepository _grants;
    private readonly IIdGenerator _idGenerator;
    private readonly ILicensingUnitOfWork _unitOfWork;

    public ClearGrantTenantOverrideHandler(
        IValidator<ClearGrantTenantOverrideCommand> validator,
    ...
    public async Task<Result<GrantTenantEntitlementsResponse>> Handle(
        ClearGrantTenantOverrideCommand command,
        CancellationToken ct)
    {
        ...
        if (!validation.IsValid)
        {
            ...
            return Result.Failure<GrantTenantEntitlementsResponse>(
                new Error("license.tenant.entitlements.clear.validation", message, ErrorType.Validation));
        }
        ...
        var cleared = tenant.ClearOverride(command.OperatorId, utcNow);
        if (cleared.IsFailure)
        {
            return Result.Failure<GrantTenantEntitlementsResponse>(cleared.Error!);
        }
